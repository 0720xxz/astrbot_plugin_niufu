...
class UniversalServerPlugin(Star):
    ...
    @filter.command("牛服")
    async def cmd_niufu(self, event: AstrMessageEvent, *_, **__):
    ...
    @filter.command("鸽服")
    async def cmd_pigeon(self, event: AstrMessageEvent, *_, **__):
        if self._is_blacklisted(event): return
        self._log_command(event, "/鸽服")
        self._trigger_active_refresh()
        ge_groups = list(dict.fromkeys([s["group"] for s in GLOBAL_DATA["servers"] if "鸽" in s["group"]]))
    ...
    @filter.command("查服")
    async def query_generic_group(self, event: AstrMessageEvent, *_, **__):
        if self._is_blacklisted(event): return
    ...
    @filter.command("ip")
    async def ip_cmd(self, event: AstrMessageEvent, *_, **__):
        if self._is_blacklisted(event): return
        self._trigger_active_refresh()
        msg = event.get_message_str().strip().split(maxsplit=1)
    ...
    @filter.command("help")
    async def help_cmd(self, event: AstrMessageEvent, *_, **__):
        if self._is_blacklisted(event): return
...
- 无斜杠触发（需管理员开启）：直接发送组名（如“牛”）即可查询
 ...
 所有开关状态均保存在插件目录下的json文件中，重启机器人后依然有效。"""
    ...
    async def enable_noslash(self, event: AstrMessageEvent, *_, **__):
    ...
    async def disable_noslash(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
        parts = event.get_message_str().strip().split()
        if len(parts) < 2:
    ...
    async def list_all_servers(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
    ...
    @filter.command("添加服")
    async def add_server(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
    ...
    @filter.command("删除服")
    async def del_server(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
    ...
    async def set_group_headers(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
        msg = event.get_message_str().strip().split(maxsplit=2)
        if len(msg) < 3:
    ...
    @filter.command("改服ID")
    async def change_server_id(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
    ...
    @filter.command("改服名")
    async def change_server_display(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
        msg = event.get_message_str().strip().split(maxsplit=3)
        if len(msg) < 4:
    ...
    @filter.command("改服组")
    async def change_server_group(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
        msg = event.get_message_str().strip().split(maxsplit=3)
        if len(msg) < 4:
    ...
    @filter.command("启用端口")
    async def enable(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
    ...
    @filter.command("禁用端口")
    async def disable(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
        msg = event.get_message_str().strip().split()
        if len(msg) < 2:
    ...
    @filter.command("调整刷新")
    async def change_refresh_rate(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
        msg = event.get_message_str().strip().split()
        if len(msg) < 2:
    ...
    @filter.command("黑名单")
    async def handle_blacklist_cmd(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
        msg = event.get_message_str().strip().split()
        if len(msg) < 3:
    ...
    @filter.command("绑定组")
    async def bind_group_cmd(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
    ...
    @filter.command("解绑组")
    async def unbind_group_cmd(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
        msg_parts = event.get_message_str().strip().split()
        if len(msg_parts) == 1:
    ...
    async def enable_fuzzy_match(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
        msg_parts = event.get_message_str().strip().split()
        if len(msg_parts) == 1:
    ...
    async def disable_fuzzy_match(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
        msg_parts = event.get_message_str().strip().split()
        if len(msg_parts) == 1:
    ...
    async def cmd_niulog(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
    ...
    @filter.command("牛服日志")
    async def cmd_niulog_cn(self, event: AstrMessageEvent, *_, **__):
        async for chunk in self.cmd_niulog(event):
            yield chunk

    @filter.command("清除日志")
    async def cmd_clear_logs(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
    ...
    @filter.command("历史")
    async def cmd_history(self, event: AstrMessageEvent, *_, **__):
    ...
    @filter.command("调整显示")
    async def cmd_adjust_count(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
    ...
    @filter.command("统计")
    async def cmd_stats(self, event: AstrMessageEvent, *_, **__):
    ...
    @filter.command("日志")
    async def cmd_logsearch(self, event: AstrMessageEvent, *_, **__):
    ...
    @filter.command("撤回时间")
    async def cmd_retract_time(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
        parts = event.get_message_str().strip().split()
        if len(parts) < 2:
    ...
    @filter.command("tg设置")
    async def cmd_tg_token(self, event: AstrMessageEvent, *_, **__):
        if not await self._is_admin(event):
            return
